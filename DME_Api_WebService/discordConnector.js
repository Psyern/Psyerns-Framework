...
const {isArray, isObject,NormalizeToGUID,GenerateLimiter, GetClientIP} = require('./utils')
...
// discord.js 14: Intents object -> GatewayIntentBits array; string permission
// names -> PermissionFlagsBits; channel type strings -> ChannelType; event name
// strings -> Events enum.
...
// ---- discord.js v14 interop helpers ----
// Map the wire channel "type" string (default "text", or v13 SCREAMING_SNAKE) to
// a v14 ChannelType enum. The wire field names themselves are unchanged.
...
function MapChannelType(type){
    ...
    if (typeof type === 'string'){
        let t = CHANNEL_TYPE_MAP[type.toLowerCase()];
        if (t !== undefined) return t;
    }
    return ChannelType.GuildText;
}
// v14 dropped the SCREAMING_SNAKE permission-string aliases. Convert the wire
// permission names (DSPerms.*, e.g. "VIEW_CHANNEL") to PermissionFlagsBits.
const PERMISSION_ALIASES = {
    UseVad: 'UseVAD',
    SendTtsMessages: 'SendTTSMessages',
    ManageEmojis: 'ManageGuildExpressions'
};
function ResolvePermission(p){
    if (typeof p === 'bigint') return p;
    if (typeof p === 'number') return BigInt(p);
    if (typeof p !== 'string') return undefined;
    if (PermissionFlagsBits[p] !== undefined) return PermissionFlagsBits[p];
    let pascal = p.toLowerCase().split('_').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('');
    if (PermissionFlagsBits[pascal] !== undefined) return PermissionFlagsBits[pascal];
    let alias = PERMISSION_ALIASES[pascal];
    if (alias !== undefined && PermissionFlagsBits[alias] !== undefined) return PermissionFlagsBits[alias];
    return undefined;
}
function ConvertPermissionArray(arr){
    if (!isArray(arr)) return undefined;
    let out = [];
    arr.forEach(p => {
        let bit = ResolvePermission(p);
        if (bit !== undefined) out.push(bit);
    });
    return out;
}
function ConvertPermissionOverwrites(overwrites){
    if (!isArray(overwrites)) return undefined;
    let out = [];
    overwrites.forEach(ow => {
        if (ow === undefined || ow === null) return;
        let entry = { id: ow.id };
        let allow = ConvertPermissionArray(ow.allow);
        let deny = ConvertPermissionArray(ow.deny);
        if (allow !== undefined) entry.allow = allow;
        if (deny !== undefined) entry.deny = deny;
        out.push(entry);
    });
    return out;
}
function BuildCreateOptions(name, options){
    let opts = { name: name };
    if (options !== undefined && options !== null){
        opts.type = MapChannelType(options.type);
        if (options.topic !== undefined) opts.topic = options.topic;
        if (options.nsfw !== undefined) opts.nsfw = options.nsfw;
        if (options.parent !== undefined && options.parent !== "") opts.parent = options.parent;
        if (options.reason !== undefined) opts.reason = options.reason;
        if (options.position !== undefined && options.position >= 0) opts.position = options.position;
        if (options.rateLimitPerUser !== undefined && options.rateLimitPerUser >= 0) opts.rateLimitPerUser = options.rateLimitPerUser;
        let po = ConvertPermissionOverwrites(options.permissionOverwrites);
        if (po !== undefined) opts.permissionOverwrites = po;
    } else {
        opts.type = ChannelType.GuildText;
    }
    return opts;
}
function BuildEditOptions(options, reason){
    let opts = {};
    if (options !== undefined && options !== null){
    ...
    if (reason !== undefined && reason !== null) opts.reason = reason;
    return opts;
}
...
// v14 removed the 'disconnect' event -> use ShardDisconnect.
    ...
    if (CheckServerAuth(auth) || (await CheckPlayerAuth(GUID, auth))){
        ...
        if (dsInfo !== undefined && dsInfo.id !== undefined &&  dsInfo.id !== "0"){
            try {
                ...
                if (CheckServerAuth(auth) || (channel.permissionsFor(player).has(PermissionFlagsBits.ViewChannel) && channel.permissionsFor(player).has(PermissionFlagsBits.Connect) )) {
                    try {
    ...
    if (CheckServerAuth(auth) || (await CheckPlayerAuth(guid, auth))){
        ...
        if (userObj !== undefined && userObj !== null && userObj.id !== "0"){
            try {
                ...
                //console.log(e);
                let error = `${e}`;
                // v14 uses numeric error codes; 50007 = "Cannot send messages to this user".
                if (e.code === 50007){
...
async function CreateChannel(res, req, auth){
    if (CheckServerAuth(auth) || (await CheckAuth(auth) && global.config.AllowClientWrite)){
        try{
            ...
            // discord.js 14: create({ name, type, ...opts }) with ChannelType enum
            // and PermissionFlagsBits-based overwrites (mapped from the wire shape).
            let channel = await guild.channels.create(BuildCreateOptions(RawData.Name, options))
            let id = channel.id;
    ...
    if (CheckServerAuth(auth) || (await CheckAuth(auth) && global.config.AllowClientWrite)){
        try{
            ...
            try {
                let channel = await guild.channels.cache.get(id);
                if (channel){
                    // discord.js 14: edit() takes a single options object (reason included).
                    channel.edit(BuildEditOptions(options, reason)).then(()=>{
...
async function SendMessageChannel(res, req, id, auth){
    ...
    if ( isServerAuth || isClientAuth){
        try{
            ...
            let guild = await client.guilds.fetch(global.config.Discord.Guild_Id);
            let cid = id;
            // discord.js 14: { embed: X } -> { embeds: [X] }. The embed JSON field
            // names from the client are passed through unchanged (wire-critical).
            let message = {embeds: [RawData]};
            ...
            try {
                ...
                if (channel && user){
                    let perms = false;
                    if (isClientAuth){
                        perms = (channel.permissionsFor(user).has(PermissionFlagsBits.ViewChannel) && channel.permissionsFor(user).has(PermissionFlagsBits.SendMessages));
                    }
                    //console.log(perms)
        ...
        try{
            ...
            try {
                ...
                if (channel && user){
                    let perms = false;
                    if (isClientAuth){
                        perms = (channel.permissionsFor(user).has(PermissionFlagsBits.ViewChannel) && channel.permissionsFor(user).has(PermissionFlagsBits.ReadMessageHistory));
                    }
                    if (isServerAuth || (isClientAuth && perms)){
                        // v14 removed Collection#array() — spread the values instead.
                        let messages = [...(await channel.messages.fetch(filter)).values()];
